...
X:${globalCoord[0].toFixed(
      2
    )} Y:${globalCoord[1].toFixed(2)} 
x:${bBox.x.toFixed(2)} y:${bBox.y.toFixed(
      2
  ...
  __highlight(el) {
    ...
    self.__highlightDom.style.left = `${(zrInsDomBBox.x + globalCoord[0]).toFixed(2)}px`;
    self.__highlightDom.style.top = `${(zrInsDomBBox.y + globalCoord[1]).toFixed(2)}px`;
  ...
  __showTooltip(el, event) {
    ...
    self.__highlightTooltipDom.innerHTML = `
      ...
      <p style="margin: 0 0 8px 0;">x: ${bBox.x.toFixed(2)} y: ${bBox.y.toFixed(2)}</p>
      <p style="margin: 0 0 8px 0;">width: ${bBox.width.toFixed(2)} height: ${bBox.height.toFixed(
      2
    )}</p>
